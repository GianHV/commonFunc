...
//        peek();
  ...
  private static void allMatch() {
    ...
    List<Order> mobile = MockUp.orderList().stream()
        .filter(order -> Objects.equals(order.getProductType(), "Mobile")).toList();
  ...
  /**
   * boolean allMatch(Predicate<? super T> predicate) It returns any element of this stream that
   * matches the provided predicate. If the stream is empty then false is returned and the predicate
   * is not evaluated.
   */
  private static void anyMatch() {
    boolean flag = MockUp.orderList().stream()
        .anyMatch(product -> product.getProductName().equals("MacBook pro"));
  ...
  private static void collect() {
    ...
    List<Order> samsung = MockUp.orderList().stream()
        .filter(order -> Objects.equals(order.getVendor(), "Samsung")).collect(Collectors.toList());
    System.out.println(samsung);

    System.out.println("\nCollectors.toSet(): All Vendors");
    Set<String> vendors = MockUp.orderList().stream().map(Order::getVendor)
        .collect(Collectors.toSet());
    System.out.println(vendors);
  }

  /**
   * static <T> Stream<T> concat(Stream<? extends T> a, Stream<? extends T> b) It creates a lazily
   * concatenated stream whose elements are all the elements of the first stream followed by all the
   * elements of the second stream. The resulting stream is ordered if both of the input streams are
   * ordered, and parallel if either of the input streams is parallel. When the resulting stream is
   * closed, the close handlers for both input streams are invoked.
   */
  ...
  /**
   * Stream<T> distinct() It returns a stream consisting of the distinct elements (according to
   * Object.equals(Object)) of this stream.
   */
  private static void distinct() {
    List<String> productType = MockUp.orderList().stream().map(Order::getProductType).distinct()
        .collect(Collectors.toList());
    System.out.println(productType);
  }

  /**
   * Optional<T> findAny() It returns an Optional describing some element of the stream, or an empty
  ...
  /**
   * Optional<T> findFirst() It returns an Optional describing the first element of this stream, or
   * an empty Optional if the stream is empty. If the stream has no encounter order, then any
   ...
   * It returns a stream consisting of the results of replacing each element of this stream with the
   * contents of a mapped stream produced by applying the provided mapping function to each element.
   * Each mapped stream is closed after its contents have been placed into this stream. (If a mapped
   * stream is null an empty stream is used, instead.)
   */
  private static void flatMap() {
    List<Order> orders = MockUp.orderList();
    List<String> productName = orders.stream().map(Order::getProductName)
        .collect(Collectors.toList());
    List<String> productType = orders.stream().map(Order::getProductType)
        .collect(Collectors.toList());
    ...
    List<String> distinctProducts = product.stream()
    ...
    productName.stream().flatMap(s -> Stream.of(s.toUpperCase().charAt(0)))
        .forEach(System.out::println);
  }

  /**
   * static <T> Stream<T> generate(Supplier<T> s) It returns an infinite sequential unordered stream
   * where each element is generated by the provided Supplier. This is suitable for generating
  ...
  private static void common() {
    ...
    Double totalSalary4 = personList.stream().mapToDouble(Order::getPrice).sum();
  ...
  /**
   * Stream<T> skip(long n) It returns a stream consisting of the remaining elements of this stream
   * after discarding the first n elements of the stream. If this stream contains fewer than n
  ...
  private static void peek() {
    MockUp.orderList().stream()
        .peek(order -> order.setProductName(order.getProductName().toUpperCase()))
        .forEach(System.out::println);
  }
}
